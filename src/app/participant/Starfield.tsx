...
type StarArray = [number, number, number, number, number, number, number, boolean];
...
interface StarData {
  ...
  y: number;
  z: number;
  star: { colorRatio: number; arr: StarArray[] };
  prevTime: number;
}
    ...
    if (sd.current.cw !== sd.current.w || sd.current.ch !== sd.current.h) {
      ...
      if (!sd.current.star.arr.length) {
        ...
        sd.current.star.arr = sd.current.star.arr.map((star, i) => {
          const newStar = [...star] as StarArray;
    ...
    sd.current.star.arr.forEach(star => {
      if (
        ...
        ctx.lineWidth = Math.max(
          (1 - sd.current.star.colorRatio * star[2]) * 5,
          1.5
        );
        ctx.beginPath();
        ...
        ctx.arc(
          ...
          star[4],
          Math.max(
            (1 - sd.current.star.colorRatio * star[2]) * 2.5,
            1
          ),
          0,
          Math.PI * 2
